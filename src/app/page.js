...
export default function PendaftaranLari() {
  const [formData, setFormData] = useState({
    namaLengkap: "", nik: "", email: "", whatsapp: "", 
    jenisKelamin: "", golonganDarah: "", namaDarurat: "", hpDarurat: "", kodePromo: "",
  });
  ...
  // State untuk Harga Dinamis di Layar
  const [ticketPrice, setTicketPrice] = useState(25000); // Default Early Bird
  const adminFee = 4000;

  // Cek tanggal saat halaman dimuat untuk menyesuaikan harga di UI
  useEffect(() => {
    const today = new Date();
    // Batas Early Bird adalah 30 Juli 2026. Mulai 1 Agustus jadi Normal.
    if (today >= new Date('2026-08-01T00:00:00+07:00')) {
      setTicketPrice(30000); 
    }
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 1. Tembak data form ke backend kita untuk mendapatkan Token
      ...
      // 2. Panggil Popup Midtrans dengan Token dari Backend
      window.snap.pay(data.token, {
        onSuccess: function(result) {
          alert("Pembayaran Berhasil! Tiket akan dikirim ke email Anda.");
          // Anda bisa menambahkan redirect ke halaman sukses di sini nanti
        },
        onPending: function(result) {
          alert("Menunggu Pembayaran. Silakan selesaikan transfer Anda.");
        },
      ...
      <div className="min-h-screen bg-[#f4f7fb] pb-24 font-sans text-gray-800">
        ...
        <header className="bg-gradient-to-br from-blue-900 to-blue-700 pt-12 pb-16 px-6 text-center text-white rounded-b-[2.5rem] shadow-lg">
          ...
          <p className="text-blue-100/90 text-sm md:text-base max-w-lg mx-auto leading-relaxed">
            Rasakan sensasi lari dengan konsep Cafe Run. Pelepasan peserta diiringi DJ mobil dan pengalaman yang tak terlupakan.
          </p>
        </header>
            ...
            <div className="bg-blue-50/50 rounded-xl p-5 border-l-4 border-blue-600">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="bg-white p-4 rounded-xl border border-blue-100 shadow-sm">
                  ...
                  <p className="text-xs text-gray-500 mt-1">Pelepasan: 06.00 WIB</p>
                </div>
                <div className="bg-white p-4 rounded-xl border border-blue-100 shadow-sm">
                  ...
                  <p className="text-xs text-gray-500 mt-1">Hiburan: DJ, Doorprize</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4">
              ...
              <div>
                ...
                <input type="text" name="namaLengkap" required value={formData.namaLengkap} onChange={handleChange} className={inputStyle} />
              </div>
              <div>
                <label className={labelStyle}>NIK KTP (16 Digit) <span className="text-red-500">*</span></label>
                <input type="tel" name="nik" required minLength="16" maxLength="16" pattern="\d*" value={formData.nik} onChange={handleChange} className={inputStyle} />
              </div>
              <div>
                <label className={labelStyle}>Email Aktif <span className="text-red-500">*</span></label>
                <input type="email" name="email" required value={formData.email} onChange={handleChange} className={inputStyle} />
              </div>
              <div>
                <label className={labelStyle}>Nomor WhatsApp <span className="text-red-500">*</span></label>
                <input type="tel" name="whatsapp" required value={formData.whatsapp} onChange={handleChange} className={inputStyle} />
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4">
              ...
              <div>
                ...
                <input type="text" name="namaDarurat" required value={formData.namaDarurat} onChange={handleChange} className={inputStyle} />
              </div>
              <div>
                <label className={labelStyle}>No. HP Darurat <span className="text-red-500">*</span></label>
                <input type="tel" name="hpDarurat" required value={formData.hpDarurat} onChange={handleChange} className={inputStyle} />
              </div>
            </div>

            {/* RINGKASAN PEMBAYARAN (DINAMIS) */}
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-blue-200 bg-blue-50/40 space-y-3">
              <h4 className="text-lg font-extrabold border-b pb-2 border-blue-100">Ringkasan Tiket</h4>
              <div className="flex justify-between items-center text-sm text-gray-600">
                <span>Tiket 5K Fun Run {ticketPrice === 25000 ? "(Early Bird)" : "(Normal)"}</span>
                <span className="font-semibold text-gray-800">Rp {ticketPrice.toLocaleString('id-ID')}</span>
              </div>
              <div className="flex justify-between items-center text-sm text-gray-600">
                <span>Biaya Layanan & Platform</span>
                <span className="font-semibold text-gray-800">Rp {adminFee.toLocaleString('id-ID')}</span>
              </div>
              <div className="flex justify-between items-center text-lg font-black text-blue-900 border-t border-blue-100 pt-3 mt-1">
                <span>Total Bayar</span>
                <span>Rp {(ticketPrice + adminFee).toLocaleString('id-ID')}</span>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-6">
              ...
              <label className="flex items-start gap-3 cursor-pointer bg-gray-50 p-4 rounded-xl border border-gray-200">
                <input 
                  ...
                  onChange={(e) => { setIsWaiverChecked(e.target.checked); if(e.target.checked) setError(""); }}
                />
                <span className="text-xs text-gray-600 leading-relaxed">
                  Saya menyatakan sehat fisik. Saya membebaskan penyelenggara dari tuntutan medis, dan setuju bahwa tiket <strong className="text-gray-900">tidak dapat dikembalikan (No Refund)</strong>.
                </span>
              </label>
            </div>
